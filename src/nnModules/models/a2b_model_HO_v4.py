...
class QuadChannelwiseInputDecompNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(f"levels: {levels}, in_channels: {in_channels}, ch_divider:{ch_divider}, n_groups:{self.n_groups}")
...
class NonCasualWaveNetBlock(nn.Module):
    def __init__(
        ...
        self.equalize_channels = (
            nn.Conv1d(self.out_channels, self.in_channels, kernel_size=1, bias=self.bias)
            if self.in_channels != self.out_channels
            else nn.Identity()
        )
        self.act = ClampedSineAct()
    ...
    def forward(self, waveform: th.Tensor) -> th.Tensor:
        ...
        if self.intermediate:
            ...
            for i in range(1, len(self.output_stages) + 1):
                this_sum = self.out_act(th.sum(th.stack(skips[:i], dim=0), dim=0) / math.sqrt(i))
                out_final.append(self.output_stages[i - 1](this_sum))
            return {"output": out_final[-1], "intermediate": out_final[:-1]}
        else:
            this_sum = self.out_act(th.sum(th.stack(skips, dim=0), dim=0) / math.sqrt(len(skips)))
